...
# Airflow Docker Deployment Script
# Version: 1.0
...
# ./deploy_airflow.sh 3.2.1

# # Check status
# ./deploy_airflow.sh --status

# # Uninstall
# ./deploy_airflow.sh --uninstall
# 
# # Show help
...
setup_host_directory() {
    ...
    if [ -d "dags" ] && [ "$(ls -A dags 2>/dev/null)" ]; then
        ...
        cp -r dags/* "$HOST_AIRFLOW_HOME/dags/" 2>/dev/null || true
...
Usage: $0 [AIRFLOW_VERSION] [OPTIONS]
...
Examples:
    ...
    $0 3.2.1                    # Deploy specific version
...
main() {
    ...
    echo "🚀 Apache Airflow Deployment Script"
